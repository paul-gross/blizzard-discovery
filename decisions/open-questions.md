...
Decided (D-030): the deliver node executes at the hub, which lands branch artifacts through an internal merge queue, rejecting stale-epoch submissions. The stubs are now mostly filled: the queue is strict FIFO, one chunk at a time across all its repos, no train batching (D-057); a merge/rebase conflict routes the chunk to the conventional `merge` graph, or to its own graph's entry node when none exists, and a graph may inline its own `merge` node feeding deliver (D-058); PR-mode merging is capability-gated on the delivery binding's merge-via-API (D-059); and branch-push fencing is deliberately absent — the pointer's commit hash is authoritative (D-060). Still open: how the hub detects an externally-merged PR so the chunk completes (D-059 wants ingestion; webhook, polling, and check-at-submission are all candidates), and how long the runner holds a chunk's environments while the hub's deliver node runs.
