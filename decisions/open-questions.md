...
The concrete values for lease TTL, heartbeat interval, the default retry cap when a node's `retries` doesn't specify one (currently 2 — the node's own `retries.max` is authoritative, [design/domain/graph.md](../design/domain/graph.md)), the tick interval (currently ~30s), and the runner-liveness heartbeat cadence and offline staleness threshold (D-070 — much slower than the worker heartbeat; ~60s and ~3 missed intervals as starting points).
