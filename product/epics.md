...
|------|------------|------------|
| `epic:store` | The runner's embedded database behind its local API: atomic claims, TTL leases, epochs, the facts-only schema. | [design/runner/store.md](../design/runner/store.md) |
...
| `epic:board` | The web board / PWA: fleet visibility plus limited operator controls; viewer and operator roles. | [design/hub/index.md](../design/hub/index.md) |
...
| `epic:team` | Team mode: shared visibility and multi-operator policy over one hub-arbitrated fleet — cross-machine arbitration is the hub's queue (D-024), never forge-native claims. | *(design doc pending; roadmap sketch in [product/roadmap.md](./roadmap.md))* |
