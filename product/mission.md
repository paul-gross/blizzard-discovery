...
| **Work source** | The project-management system defining what work exists and its priority — plugs into the hub (D-024), never into runners | GitHub issues + labels |
...
2. **Zero duplicated or clobbered work.** No two agents ever hold the same task, and no reaped-but-still-running agent ever overwrites its successor's delivery. These are structural guarantees (atomic claims, epochs), not probabilistic ones.
...
5. **Interoperable by construction.** From `milestone:mvp`, every component works against a seam's interface, never its concrete binding — the reference stack is the first implementation of the interfaces, not a shortcut around them (D-016). Swapping a binding — a different forge, harness, or workspace provider — is an adapter's worth of work, never a rewrite; the claim is proven the first time a seam has two live bindings (harnesses will be first: Claude Code, then OpenCode/Codex).
