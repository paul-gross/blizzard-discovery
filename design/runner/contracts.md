...
| Operation | In | Out | Notes |
|-----------|-----|-----|-------|
| `spawn` | env ids + workdirs, prompt, session hint | `{session_id, pid}` | Session identity may be harness-assigned (D-092): the hint is honored where supported (Claude Code), else the returned id is recorded at spawn-return; pid + process start time are recorded by the spawn wrapper from inside the child. The prompt is the hub's node envelope plus the runner's machine-local preamble — held env ids and workdirs (D-063); `BLIZZARD_ENV_IDS` joins the injected identity variables. Every adapter ships an executable per-harness conformance suite (D-092). |
...
| claim work | runner → hub | FILL peeks the ready queue read-only, then claims by `POST /routes` carrying the acquired env ids (D-080) — `409` = race-second-place; the claim response carries the first **node envelope** (prompt, node config, all artifacts latest-by-epoch — D-089), and `GET /chunks/{id}/envelope` is the idempotent re-read after a lost apply-response (D-090). |
...
Delivery is the deliver node of the workflow graph (D-025), executed **at the hub** through its merge queue with the epoch check (D-030; [concurrency-model.md](../concurrency-model.md)); baseline is merge to main, and a human gate is a node inserted ahead of deliver. The runner's part ends at pushed branch pointers and the transition into the deliver node, though it holds the chunk's environments until the hub reports the terminal outcome (D-066). The mechanics are settled (FIFO queue D-057, conflict routing — intra-graph in the MVP, D-086/D-058 — capability-gated PR merging D-059, external-merge detection D-065, per-repo serialized reconciliation D-091, coordinator execution D-079); the delivery-seam operations themselves (land a chunk's branch artifacts, open/track/check a PR, surface a gated chunk) still need their per-operation contract shape — see the seam-interface-contracts [open question](../../decisions/open-questions.md).
