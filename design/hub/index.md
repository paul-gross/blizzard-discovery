...
- **Wraps the project-management layer (D-024).** The PM binding — GitHub issues is one — plugs into the hub, which ingests ready PM items into its own internal **chunks** and reflects state and completion back. Runners never speak to the PM system; when a runner wants work, it asks the hub. Queue **prioritization is a hub requirement**: GitHub has no per-issue priority, so the hub must implement ordering itself — mechanism [open](../../decisions/open-questions.md). Hub-side routing (pinning work to a runner) also lives here, post-MVP.
...
- **Serves the clients.** HTTP API + SSE for the CLI's fleet verbs today, the board and chat bot later; the auth seam (D-018) guards this surface.
...
A runner's connection is **outbound-only** (D-012): runners poll the hub or subscribe to its SSE stream; **nothing dials into the dev box** — the same pattern as Anthropic's self-hosted sandbox workers, where the sensitive machine only makes outbound connections. The hub is **eventually reachable**: during an outage, runners keep working on already-claimed chunks, buffer events locally, and flush when it returns.
...
## Clients: the board and the chat bot

The **board** is a server-rendered page plus SSE, phone-friendly as a PWA. It is mostly a visibility surface, but not read-only: two roles split it — **viewer** (sees everything, touches nothing) and **operator** (answers questions and drives limited controls; the exact control set is an [open question](../../decisions/open-questions.md)).
