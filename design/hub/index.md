...
- **Wraps the project-management layer (D-024/D-047).** Hub configuration declares zero or more named **PM sources** — `[[pm_source]]`, each `{name, provider, repo, token_env}` plus optional `api_base`/`web_base` (D-106) — and the hub builds one credentialed binding per source at composition, never a shared client or token. Work enters by **explicit, id-addressed ingestion** via `blizzard hub ingest` / `POST /chunks` (D-074): ingesting an item stores a **pointer** — `{source, ref}`, the configured source's name plus the item's source-relative reference (D-105), superseding the old `{provider, url}` shape — and mints a chunk by default, one per item; a pointer naming an unconfigured source is rejected at ingest (D-107). Item contents are read **pass-through**: anything that needs the guts (the build node included) fetches them through the hub's API — body and comment thread, vendor-native format (D-074) — and the hub calls the PM system with the resolved source's own credentials; PM credentials never reach a runner; the payload's consumers are LLMs. Selection — which items are fleet-ready — happens outside the hub: humans or agents ingest specific ids (the agent-native flow is a documented pattern, not shipped machinery); there is no label sweep, and the source list bounds which bindings exist, not which items are ready (D-107). Chunks are ephemeral: any unacquired chunk may be discarded — real deletion; acquired-with-history chunks are stopped instead (D-067/D-074) — and re-ingesting the same pointer mints a fresh chunk only once every prior holder is terminal: a pointer with a live chunk is rejected at ingest, 409 with the existing chunk id (D-093). The MVP writes nothing back to the PM item. Runners never speak to the PM system; when a runner wants work, it asks the hub. Queue **prioritization is a hub-side property** — mechanism [open](../../decisions/open-questions.md). Hub-side routing (pinning work to a runner) also lives here, post-MVP.
