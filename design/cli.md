...
| `blizzard runner pm-items <chunk-id>` | worker | `GET /chunks/{id}/pm-items` (local proxy) | Pass-through read of the chunk's PM items, proxied runner → hub → vendor (D-084) — a worker never crosses a layer. |
...
| `blizzard hub ingest …` | human | `POST /chunks` | Ingest PM items by a source-native token per item — `source:ref`, `source#ref`, or the item's own URL, resolved hub-side against every configured source's own `parse` (D-108/D-109); the CLI carries no token grammar of its own. Specific ids always, batch fine; a token no configured source claims is rejected (D-107/D-109), and a resolved pointer with a live chunk is rejected (D-093). The chunk rests `not_ready` (D-103) until promoted. |
| `blizzard hub promote <chunk-id>` | human | `POST /chunks/{id}/promote` | Promote a `not_ready` chunk to `ready` so a runner may claim it (D-103); idempotent — re-promoting a ready chunk is a no-op. |
