...
| `blizzard hub ingest …` | human | `POST /chunks` | Ingest PM items by a source-native token per item — `source:ref`, `source#ref`, or the item's own URL, resolved hub-side against every configured source's own `parse` (D-110/D-111); the CLI carries no token grammar of its own. Specific ids always, batch fine; a token no configured source claims is rejected (D-109/D-111), and a resolved pointer with a live chunk is rejected (D-093). The chunk rests `not_ready` (D-103) until promoted. |
