...
| `blizzard hub ingest …` | human | `POST /chunks` | Ingest PM items by a `source:ref` token per item — the configured source's name plus the item's reference (D-105/D-106); specific ids always, batch fine; a pointer naming an unconfigured source is rejected (D-107), and a pointer with a live chunk is rejected (D-093). The chunk rests `not_ready` (D-103) until promoted. |
