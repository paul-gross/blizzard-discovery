...
| `blizzard hub ingest …` | human | `POST /chunks` | Ingest PM items by pointer — `{provider, url}` per item (D-074/D-075); specific ids always, batch fine; a pointer with a live chunk is rejected (D-093). The chunk rests `not_ready` (D-103) until promoted. |
