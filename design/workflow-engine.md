...
- **Human gates are nodes — and a runner-config dial (D-032).** A gate is a node whose "judgement" is a person; a chunk parked there holds an open **Decision** — a durable multiple-choice row carrying the step's artifacts (D-045) — and derives `waiting_on_human` with its reap clock stopped, exactly like an ask (D-010/D-015 machinery reused); the person's choice resolves it, and the holding runner records the transition. Mechanically the two gate levers are one mechanism (D-045): a *graph* gate is the hub refusing a transition at a human-judged node, and a *runner-config* gate is the runner choosing to submit a Decision instead of a transition for a node it was configured to gate. Besides gate nodes in the graph, a runner can be **configured to impose a human gate on any node by node name** (D-041 — names span graphs by convention), so an individual operator dials their own HITL level without forking the fleet's graph; the shape is runner-local declarative config — `gates = ["deliver", …]` in the runner's own config, matched across all graphs, reloaded on tick, effective gating the union of graph and runner gates (D-073). This retires the separate gate-matrix mechanism — D-014's autonomous baseline survives as *the default graph containing no gate nodes*.
...
The **runner holding a chunk drives it; the hub applies the rules** (D-072): when a node-step finishes, the runner evaluates the judgement in-session and submits the completion — judgement choice, check results, artifacts — through its outbound buffer; the hub's **apply-response** records the transition (and any pending migration, atomically in the same write — this is where deferred migrations apply) and returns the chunk's **next node envelope**, which is what lets the runner continue in place **without re-queuing** — same environment, artifacts already in the worktree, session resumable. The runner holds no graph definitions: a migrated chunk's swap is discovered in the returned envelope's graph/node ids, never inferred. Verdict-less failures (D-009) retry runner-locally from the envelope's own retries config — no round trip. Advancement therefore needs hub reachability: during an outage chunks finish their current node and wait at the boundary, workers mid-node unaffected — milliseconds-cheap colocated (D-022). Whether the next node gets a fresh agent or a resumed session is per-node graph configuration, defaulting to resume (D-054). Hub nodes are the exception: when the transition lands on one (deliver, D-030), the hub executes it — the runner holds the chunk's environments until the hub reports the outcome (D-066).
...
When a runner picks up a node-step it receives a **node envelope**: the prompt, the node config, and the chunk's relevant artifacts. The envelope is pure-LLM (D-053): every artifact is presented in the prompt — pointer artifacts as their commit hash and feature branch name — and the agent sets up its own workspace from them. Agents manage environments, not the other way around; any pre-materialization is a configurable property of the runner↔workspace seam, never engine behavior.
...
The default graph ends in a **deliver** node — the first **hub-executed node**. The chunk's branch artifacts are already pushed to the forge by the time the chunk reaches it (D-026), so the hub can land them without ever holding code: it merges to main (or opens the pull request, where a gate wants one) through an internal **merge queue** that processes one chunk at a time. The delivery lane's single-writer property therefore lives at the hub, where it can serialize deliveries across every runner in the fleet ([concurrency-model.md](./concurrency-model.md)); a submission carrying a stale epoch is rejected before it enters the queue, so a zombie's work never lands. Delivery being a node is what makes the gate story uniform — a team that wants human sign-off before merge inserts a gate node ahead of deliver, and nothing else changes. The queue is strict FIFO, one chunk at a time across all its repos, with no train batching in the MVP (D-057). A merge/rebase conflict routes the chunk to a merge workflow by convention (D-058): the conventional `merge` graph if one exists — its own prompts for merging, review, and redelivery — else the entry node of the chunk's own graph; a graph may also inline its own `merge` node routing to deliver, so the deliver node deliberately has two entrypoints. In PR mode, merging is capability-gated on the delivery binding's merge-via-API (D-059). There is no fencing at the branch ref — the pointer's commit hash is authoritative, and the branch name serves only to detect commits ahead of it (D-060). Where the hub cannot merge, the PR's fate is detected by hub-side polling (D-065): a slow tick over the chunks parked awaiting an external merge, an on-demand `POST /chunks/{id}/check-delivery` route for the impatient path, and — later — an inbound webhook that merely triggers the immediate check, so polling stays the source of truth. Any terminal PR state completes the chunk: merged or closed-without-merge, the chunk moves to delivered, with the recorded fact distinguishing the two and carrying the actually-landed commit where one exists. Throughout, the holding runner keeps the chunk's environments until the hub reports the terminal outcome — including across a PR-mode external-merge wait (D-066) — so a conflict routes back into warm environments; a landed or externally-closed delivery releases them.
