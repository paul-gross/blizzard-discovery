...
| Status | Derived from |
|--------|--------------|
| `ready` | Ingested from the PM binding into a chunk (D-024), no route recorded yet — sitting in the hub's ready queue. |
...
The unit of work that travels the workflow graph (D-024, D-025). The center of the model: everything else hangs off it.

| Property | Notes |
|----------|-------|
| `chunk_id` | Hub-minted; exact identity scheme is an [open question](../../decisions/open-questions.md). |
| PM back-reference(s) | The wrapped PM item(s) — a GitHub issue in the reference binding (D-024); plural once batching lands (open). |
...
| latest epoch | **Derived** from the chunk's newest claim fact — one claim per node-step attempt (D-035) — the fence every state-advancing write is checked against (D-007). |
...
| `epoch` | The executing claim's fencing epoch — one claim per node-step attempt (D-035) — checked against the chunk's latest epoch; stale transitions are rejected, not recorded (D-007). |
...
| `epoch` | The submitting claim's fencing epoch — stale decisions are rejected like stale transitions (D-007). |
...
| `epoch` (forced only) | The new fencing epoch minted by claim revocation — the existing epoch check (D-007) then rejects the abandoned in-flight work, which is redone under the new graph. |
